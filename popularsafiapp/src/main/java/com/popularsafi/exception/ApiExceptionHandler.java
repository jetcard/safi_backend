 ...
 * 100-level (Informational) â€” Server acknowledges a request, it mean that request was received and understood, it is transient response , alert client for awaiting response
 * 200-level (Success) â€” Server completed the request as expected
 * 300-level (Redirection) â€” Client needs to perform further actions to complete the request
 * 400-level (Client error) â€” Client sent an invalid request
 * 500-level (Server error) â€” Server failed to fulfill a valid request due to an error with server
 *
