...
@CrossOrigin(origins = {"*"})
@RestController
@RequestMapping("/reporte_provisiones/3")
@RequiredArgsConstructor
public class ReporteProvisionesControllerSACIF_MYP {
    private static final Logger logger= LoggerFactory.getLogger(ReporteProvisionesControllerSACIF_MYP.class.getName());
    @Autowired
    private final IProvisionServiceSACIF_MYP servSACIF_MYP;

    @Qualifier("provisionMapper")
    private final ModelMapper modelMapper;

    @GetMapping(path = "/json", produces= MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<ProvisionDTO>> obtenerProvisionICSACIF_MYP(@RequestParam("fechaProceso") String fechaProceso) {

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
        DateTimeFormatter formatter2 = DateTimeFormatter.ofPattern("dd/MM/yyyy");
        logger.info(LocalDate.parse(fechaProceso, formatter).format(formatter2));
        servSACIF_MYP.ejecutarFechaSACIF_MYP(LocalDate.parse(fechaProceso, formatter).format(formatter2));
        List<ProvisionDTO> listaSACIF_MYP  = servSACIF_MYP.findAll().parallelStream().map(this::convertToDto).collect(Collectors.toList());
        return new ResponseEntity<>(listaSACIF_MYP, HttpStatus.OK);
    }
    @GetMapping("/xls")
    ...
    public ResponseEntity<ProvisionResponseRest> imprimirReporteFecha() {
        ...
        try {
            List<ReporteProvision> lista  = servSACIF_MYP.findAll();
