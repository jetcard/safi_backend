...
import com.popularsafi.excel.ExcelRiesgosSACIF_MYP;
import com.popularsafi.excel.ExcelRiesgosSACIF_POP;
...
@CrossOrigin(origins = {"*"})
@RestController
@RequestMapping("/reporte_riesgos/2")
@RequiredArgsConstructor
public class ReporteRiesgoControllerSACIF_POP {
    private static final Logger logger= LoggerFactory.getLogger(ReporteRiesgoControllerSACIF_POP.class.getName());
    @Autowired
    private final IRiesgoServiceSACIF_POP servSACIF_POP;
    ...
    public ResponseEntity<List<ReporteRiesgoDTO>> obtenerReporteRiesgo(@RequestParam("fechaProceso") String fechaProceso) {
        List<ReporteRiesgoDTO> ListReporteRiesgo = servSACIF_POP.findAllByFecha(fechaProceso).stream().map(this::convertToDto).collect(Collectors.toList());
    ...
    public void exportToExcel(HttpServletResponse response, @RequestParam("fechaProceso") String fechaProceso) throws IOException {
        ...
        ExcelRiesgosSACIF_POP excelExporter = new ExcelRiesgosSACIF_POP(
    ...
    public ResponseEntity<RiesgoResponseRest> imprimirReporteFecha(String fechaProceso) {
        ...
        try {
            List<ReporteRiesgo> lista  = servSACIF_POP.findAllByFecha(fechaProceso);
