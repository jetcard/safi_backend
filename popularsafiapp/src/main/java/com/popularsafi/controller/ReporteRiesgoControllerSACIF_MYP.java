...
@CrossOrigin(origins = {"*"})
@RestController
@RequestMapping("/reporte_riesgos/3")
@RequiredArgsConstructor
public class ReporteRiesgoControllerSACIF_MYP {
    private static final Logger logger= LoggerFactory.getLogger(ReporteRiesgoControllerSACIF_MYP.class.getName());
    @Autowired
    private final IRiesgoServiceSACIF_MYP servSACIF_MYP;
    ...
    public ResponseEntity<List<ReporteRiesgoDTO>> obtenerReporteRiesgo(@RequestParam("fechaProceso") String fechaProceso) {
        List<ReporteRiesgoDTO> ListReporteRiesgo = servSACIF_MYP.findAllByFecha(fechaProceso).stream().map(this::convertToDto).collect(Collectors.toList());
    ...
    public void exportToExcel(HttpServletResponse response, @RequestParam("fechaProceso") String fechaProceso) throws IOException {
        ...
        ExcelRiesgosSACIF_MYP excelExporter = new ExcelRiesgosSACIF_MYP(
    ...
    public ResponseEntity<RiesgoResponseRest> imprimirReporteFecha(String fechaProceso) {
        ...
        try {
            List<ReporteRiesgo> lista  = servSACIF_MYP.findAllByFecha(fechaProceso);
